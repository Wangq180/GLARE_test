...
from data.LoL_dataset import LoL_Dataset, LoL_Dataset_v2, LoL_Dataset_RIDCP
...
# 在脚本最上方设置一个控制变量
only_generate_val_images = True  # 如果只生成VAL images，将此设置为True
...
def main():
    ...
    parser.add_argument('--opt', type=str, help='Path to option YMAL file.',
                            default='./code/confs/LOL-vq-no-squeeze-with-mean-320-large-lr-gt-hqps-9-17-4.yml' if sys.platform != 'win32' else './code/confs/LOL-vq-no-squeeze-with-mean-320-large-lr-gt-hqps-9-17-4.yml') #  './confs/LOLv2-pc_rebuttal.yml') # 
    ...
    if rank <= 0:  # normal training (rank -1) OR distributed training (rank 0)
        ...
        if opt.get('use_tb_logger', False) and 'debug' not in opt['name']:
            ...
            if version >= 1.1:  # PyTorch 1.1
                ...
                logger.info('You are using PyTorch {}. Tensorboard will use [tensorboard]'.format(version))
    ...
    # train_loader = torch.utils.data.DataLoader(train_set, batch_size=2, shuffle=False, num_workers=1, pin_memory=True, sampler=train_sampler)
    # val_loader = torch.utils.data.DataLoader(val_set, batch_size=1, shuffle=False, num_workers=1, pin_memory=True, sampler=test_sampler) # sampler和shuffle不能同时使用
    ...
    #### training
    #model.load_network(load_path='50000_G.pth', network=model.netG)
    timer = Timer()
    logger.info('Start training from epoch: {:d}, iter: {:d}'.format(start_epoch, current_step))
    timerData = TickTock()
    avg_psnr = best_psnr = -1
    
    #### initialize NATSchedule
    nat_schedule = NATSchedule(
        codebook_size=model.net_hq.module.codebook_size,  # 使用 .module 访问真实模型属性
    ...
    for epoch in range(start_epoch, total_epochs + 1):
        ...
        for _, train_data in enumerate(train_loader):
            ...
            nll = model.optimize_parameters(current_step, val_loader)
            ...
            if (current_step) % opt['train']['val_freq'] == 0 and rank <= 0:
                ...
                for val_data in val_loader:
                    ...
                    out_lr=visuals['NORMAL'].reshape(1, 512, 100, 150)
            
                    x, _, _ =model.net_hq.quantize_group[0](out_lr, weight_alpha=-1.0)
                    pre_quant = None
                    after_quant_feat = model.net_hq.after_quant_group[0](x, pre_quant)
                    x = model.net_hq.decoder_group[0](after_quant_feat)
                    x= model.net_hq.decoder_group[1](x)
                    out_lr= model.net_hq.out_conv(x)
                    normal_img = None
                    ...
                    if hasattr(model, 'heats') and model.heats is not None:
                        ...
                        normal_img = util.tensor2img(out_lr)  # uint8
                        save_img_path = os.path.join(img_dir,
                    ...
                    psnr_0 = util.calculate_psnr(normal_img* 255, gt_img * 255)
                    avg_psnr_0 += psnr_0
                    ...
                    save_img_adjust_path=os.path.join(img_dir,
                                                     '{:s}_{:d}_adjust.png'.format(img_name, current_step))
                    util.save_img((cropped_sr_img_adjust * 255).astype(np.uint8), save_img_adjust_path)
                    avg_psnr += psnr
                    ssim = util.ssim(visuals['GT'].unsqueeze(0), out_lr).item()
                    avg_ssim += ssim
                    line += '%s %.5f %.5f %.5f\n' % (img_name, psnr_0, psnr, ssim)

                avg_psnr = avg_psnr / idx
                avg_psnr_0 = avg_psnr_0 / idx
                avg_ssim = avg_ssim / idx
                avg_nll = sum(nlls) / len(nlls)
                with open(os.path.join(opt['path']['val_images'], str(current_step), 'metrics.txt'), 'w') as f:
                    f.write(line)
                    f.write('noadjust_PSNR:' + str(avg_psnr_0)+'     adjust_PSNR:'+ str(avg_psnr)+'     ssim:'+str(avg_ssim)+'\n')

                # log
                logger.info('# Validation # PSNR: {:.4e} SSIM: {:.4e}'.format(avg_psnr, avg_ssim))
                logger_val = logging.getLogger('val')  # validation logger
                logger_val.info('<epoch:{:3d}, iter:{:8,d}> psnr: {:.4e} SSIM: {:.4e} mask_ratio: {:.4f}'.format(
                    epoch, current_step, avg_psnr, avg_ssim, model.log_dict.get('mask_ratio', 0)))
            ...
            save_ = False
            if current_step >= 30000 and current_step % 200 == 0:
                save_ = True
            if current_step >= 40000 and current_step % 100 == 0:
                save_ = True
            if current_step >= 50000 and current_step % 50 == 0:
                save_ = True
            
            if current_step % opt['logger']['save_checkpoint_freq'] == 0 or save_:
                if rank <= 0:
