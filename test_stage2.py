...
def main():
    ...
    test_dir = os.path.join(this_dir, '..', 'results-stage2-attemp3', conf)
    ...
    for i in natsort.natsorted(os.listdir('experiments/GLARE_NAT_stage2--attempt1/models')):
        path=os.path.join('experiments/GLARE_NAT_stage2--attempt1/models', i)
        ...
        for lr_path, hr_path, idx_test in zip(lr_paths, hr_paths, range(len(lr_paths))):
            ...
            with torch.cuda.amp.autocast():
                ...
                # sr_up_right = model.get_sr(lq=lr_up_right.cuda(), heat=None)
                # sr_up_right=sr_up_right.cuda()
                # sr_up_right, _, _ = model.net_hq.module.decode(sr_up_right)
            ...
            std_out  = torch.std(lr_out.reshape(lr_out.shape[0], -1))
            std_gt = torch.std(t(hr).reshape(t(hr).shape[0], -1))
        ...
        print('Mean: ' + str_out)
        with open('NATstage2-attempt3.txt.txt','a') as f:
